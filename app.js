...
let frequency = null;
let selectedSpectrumX = N / 2;
let selectedSpectrumY = N / 2;
...
function selectSpectrumPoint(sx,sy){selectedSpectrumX=Math.max(0,Math.min(N-1,Math.round(sx)));selectedSpectrumY=Math.max(0,Math.min(N-1,Math.round(sy)));const fx=selectedSpectrumX-N/2,fy=selectedSpectrumY-N/2,i=((fy+N)%N)*N+((fx+N)%N),strength=Math.hypot(frequency.re[i],frequency.im[i]),dc=Math.max(1,Math.hypot(frequency.re[0],frequency.im[0])),strengthPct=(100*strength/dc).toFixed(2);const wave=new Float64Array(N*N);for(let y=0;y<N;y++)for(let x=0;x<N;x++)wave[y*N+x]=127.5+127.5*Math.cos(2*Math.PI*(fx*x+fy*y)/N);paint(pattern,wave);document.querySelector('#freqX').textContent=fx;document.querySelector('#freqY').textContent=fy;document.querySelector('#freqStrength').textContent=strengthPct+'% of DC';const zero=fx===0&&fy===0;document.querySelector('#patternTitle').textContent=zero?'Constant brightness':`${Math.abs(fx)} × ${Math.abs(fy)} cycle pattern`;document.querySelector('#patternCopy').textContent=zero?'The centre point is the average image brightness (zero frequency).':`This wave repeats ${Math.abs(fx)} time(s) horizontally and ${Math.abs(fy)} time(s) vertically across the image.`;spectrumStatus.textContent=`Selected frequency: ${fx} horizontal cycles, ${fy} vertical cycles, strength ${strengthPct}% of DC.`;updateSelectedMath(fx,fy,i);}
function inspectFrequency(clientX,clientY){const r=spectrum.getBoundingClientRect(),sx=Math.floor((clientX-r.left)*N/r.width),sy=Math.floor((clientY-r.top)*N/r.height);selectSpectrumPoint(sx,sy);}
spectrum.addEventListener('click',e=>inspectFrequency(e.clientX,e.clientY));
spectrum.addEventListener('keydown',e=>{const moves={ArrowLeft:[-1,0],ArrowRight:[1,0],ArrowUp:[0,-1],ArrowDown:[0,1]};if(e.key==='Home'){e.preventDefault();selectSpectrumPoint(N/2,N/2);return;}const move=moves[e.key];if(!move)return;e.preventDefault();selectSpectrumPoint(selectedSpectrumX+move[0],selectedSpectrumY+move[1]);});
...
keep.addEventListener('input',update);document.querySelector('#example').addEventListener('click',drawExample);drawExample();selectSpectrumPoint(N/2,N/2);
